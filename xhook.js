...
xhook.after((request, response) => {
    if (request.url.startsWith("/for/apiAssets/chat/emotes.json")) {
      let tempData = JSON.parse(response.text);
      tempData.hiddenEmotes = tempData.hiddenEmotes.concat(emotes);
      response.text = JSON.stringify(tempData);
    }
    return response;
  });
`);
